...
class IDLGenerator:
    ...
    def _parse_header(self):
        ...
        for k in self._parse_results:
            for enum in self._parse_results[k]["enum"]:
                self._key_list[enum["name"]] = "enum"
            for union in self._parse_results[k]["union"]:
                self._key_list[union["name"]] = "union"
            for struct in self._parse_results[k]["struct"]:
                self._key_list[struct["name"]] = "struct"
            for clas in self._parse_results[k]["interface"]:
                self._key_list[clas["name"]] = ""
            for cb in self._parse_results[k]["callback"]:
                self._key_list[cb["name"]] = ""
            for td in self._parse_results[k]["typedef"]:
